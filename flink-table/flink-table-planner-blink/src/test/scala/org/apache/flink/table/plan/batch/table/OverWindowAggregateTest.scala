...
class OverWindowAggregateTest extends TableTestBase {
  ...
  private val t = util.addTableSource[(Int, Long, String, Long)]("Table3", 'a, 'b, 'c)

  @Test
  def testOverWindowCurrentRowAndCurrentRow(): Unit = {
    val resJava = t.window(
      Over.partitionBy("c").orderBy("a").preceding("current_row").following("current_row").as("w"))
        .select("c, count(a) OVER w")

    val resScala = t
        .window(Over partitionBy 'c orderBy 'a preceding CURRENT_ROW following CURRENT_ROW as 'w)
