...
class GroupWindowStringExpressionTest extends TableTestBase {
  ...
  def testRowTimeSlide(): Unit = {
    ...
    val t = util.addDataStream[(Int, Long, String)]("T1", 'int, 'long, 'string, 'rowtime)
  ...
  def testRowTimeTumble(): Unit = {
    val util = streamTestUtil()
    val t = util.addDataStream[(Int, Long, Long, String)]("T1", 'int, 'long, 'rowtime, 'string)
  ...
  def testProcTimeSlide(): Unit = {
    val util = streamTestUtil()
    val t = util.addDataStream[(Int, Long, String)]("T1", 'int, 'long, 'string, 'proctime)
