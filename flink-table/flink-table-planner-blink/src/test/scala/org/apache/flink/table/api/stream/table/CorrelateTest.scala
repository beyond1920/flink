...
class CorrelateTest extends TableTestBase {
  ...
  def testCorrelateWithMultiFilterAndWithoutCalcMergeRules(): Unit = {
    val util = streamTestUtil()
    val programs = util.tableEnv.getConfig.getCalciteConfig.getStreamProgram
      .getOrElse(FlinkStreamProgram.buildProgram(util.getTableEnv.getConfig.getConf))
    ...
    val calciteConfig = CalciteConfig.createBuilder(util.tableEnv.getConfig.getCalciteConfig)
      .replaceStreamProgram(programs).build()
