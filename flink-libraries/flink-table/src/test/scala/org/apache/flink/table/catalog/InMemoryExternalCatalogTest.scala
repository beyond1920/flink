...
class InMemoryExternalCatalogTest {
  ...
  def testCreatePartition(): Unit = {
    ...
    assertTrue(catalog.listPartition(databaseName, tableName).isEmpty)
    ...
    val partitionSpecs = catalog.listPartition(databaseName, tableName)
  ...
  def testDropPartition(): Unit = {
    ...
    val newPartition = ExternalCatalogTablePartition(newPartitionSpec)
    catalog.createPartition(databaseName, tableName, newPartition, false)
    assertTrue(catalog.listPartition(databaseName, tableName).contains(newPartitionSpec))
    catalog.dropPartition(databaseName, tableName, newPartitionSpec, false)
    assertFalse(catalog.listPartition(databaseName, tableName).contains(newPartitionSpec))
  }
